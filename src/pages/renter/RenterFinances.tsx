...
const RenterFinances = () => {
  ...
  // ✅ Добавлены стейты для totals (как в Lessor)
  ...
  const {
    ...
    queryKey: ['renter-finances', companyId, paymentsPage, eventsPage],
    queryFn: async () => {
      const skip = (paymentsPage - 1) * PAGE_SIZE;
      const eventsSkip = (eventsPage - 1) * PAGE_SIZE;
      
      const data = await renterService.getFinances(
        companyId!,
        skip,
        PAGE_SIZE,
        eventsSkip,
    ...
    staleTime: 0,
    refetchOnMount: true,
    // ✅ Убран placeholderData - он ломал пагинацию
  });
  ...
  // ✅ Такая же логика как в Lessor
  const paymentsTotalPages = Math.max(1, Math.ceil(paymentsTotal / PAGE_SIZE) 
    || (payments.length === PAGE_SIZE ? paymentsPage + 1 : paymentsPage));
  const eventsTotalPages = Math.max(1, Math.ceil(eventsTotal / PAGE_SIZE) 
    || (balanceEvents.length === PAGE_SIZE ? eventsPage + 1 : eventsPage));
  ...
  const resetAndRefetch = async () => {
    setPaymentsPage(1);
    setEventsPage(1);
    await queryClient.invalidateQueries({ queryKey: ['renter-finances'] });
  };

  const topUpMutation = useMutation({
    mutationFn: (value: number) =>
      renterService.topUpBalance(companyId!, value),
    onSuccess: () => {
      resetAndRefetch();
      closeModal();
    },
  ...
  const withdrawMutation = useMutation({
    mutationFn: (value: number) =>
      renterService.withdrawBalance(companyId!, value),
    onSuccess: () => {
      resetAndRefetch();
      closeModal();
    },
