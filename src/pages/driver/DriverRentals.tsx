import { useState } from 'react';
import { useQuery } from '@tanstack/react-query';
import { Truck, MapPin, AlertTriangle, Loader2, Car, Calendar, Building2 } from 'lucide-react';
import { MapContainer, TileLayer, Marker, Popup } from 'react-leaflet';
import L from 'leaflet';
import 'leaflet/dist/leaflet.css';
import { driverService } from '../../services/driverService';
import { Card } from '../../components/ui/Card';
import { Badge } from '../../components/ui/Badge';
import { Modal } from '../../components/ui/Modal';
import type { RentalStatus } from '../../types';

delete (L.Icon.Default.prototype as any)._getIconUrl;
L.Icon.Default.mergeOptions({
  iconRetinaUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/marker-icon-2x.png',
  iconUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/marker-icon.png',
  shadowUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/marker-shadow.png',
});

const statusConfig: Record<RentalStatus, { label: string; variant: 'green' | 'gray' | 'red' }> = {
  ACTIVE: { label: 'Активна', variant: 'green' },
  COMPLETED: { label: 'Завершена', variant: 'gray' },
  OVERDUE: { label: 'Просрочена', variant: 'red' },
};

const DriverRentals = () => {
  const [filter, setFilter] = useState<RentalStatus | undefined>(undefined);
  const [mapModal, setMapModal] = useState<string | null>(null);
  const [violationsModal, setViolationsModal] = useState<string | null>(null);
  const [geoEventsModal, setGeoEventsModal] = useState<string | null>(null);

  const { data: company } = useQuery({
    queryKey: ['driver-company'],
    queryFn: driverService.getMyCompany,
  });

  const { data: rentals, isLoading } = useQuery({
    queryKey: ['driver-rentals', filter],
    queryFn: () => driverService.getMyRentals(filter, 0, 50),
  });

  const { data: telemetry } = useQuery({
    queryKey: ['driver-telemetry', mapModal],
    queryFn: () => driverService.getRentalTelemetry(mapModal!),
    enabled: !!mapModal,
    refetchInterval: 5000,
  });

  const { data: violations } = useQuery({
    queryKey: ['driver-violations', violationsModal],
    queryFn: () => driverService.getRentalViolations(violationsModal!),
    enabled: !!violationsModal,
  });

  const { data: geoEvents } = useQuery({
    queryKey: ['driver-geo-events', geoEventsModal],
    queryFn: () => driverService.getRentalGeofenceEvents(geoEventsModal!),
    enabled: !!geoEventsModal,
  });

  return (
    <div className="p-8">
      {/* Company info */}
      {company && (
        <div className="flex items-center gap-3 mb-6 p-4 bg-white/5 border border-white/10 rounded-2xl">
          <div className="w-10 h-10 bg-green-500/20 rounded-xl flex items-center justify-center">
            <Building2 size={18} className="text-green-400" />
          </div>
          <div>
            <p className="text-white font-semibold">{company.name}</p>
            <p className="text-gray-500 text-sm">Моя компания · ИНН: {company.inn}</p>
          </div>
        </div>
      )}

      <h1 className="text-3xl font-bold text-white mb-6">Мои аренды</h1>

      <div className="flex gap-2 mb-6">
        {([undefined, 'ACTIVE', 'COMPLETED', 'OVERDUE'] as (RentalStatus | undefined)[]).map(s => (
          <button key={s || 'all'} onClick={() => setFilter(s)}
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${filter === s ? 'bg-[#6C63FF] text-white' : 'bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white'}`}>
            {s ? statusConfig[s].label : 'Все'}
          </button>
        ))}
      </div>

      {isLoading && <div className="text-center py-12"><Loader2 size={28} className="animate-spin mx-auto text-[#6C63FF]" /></div>}

      <div className="space-y-4">
        {rentals?.map(rental => (
          <Card key={rental.id}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-3">
                  <Badge variant={statusConfig[rental.status].variant}>{statusConfig[rental.status].label}</Badge>
                  <span className="text-gray-500 text-xs">{new Date(rental.created_at).toLocaleDateString('ru-RU')}</span>
                </div>
                <div className="grid md:grid-cols-2 gap-3 mb-3">
                  {rental.car && (
                    <div className="flex items-start gap-2 p-3 bg-white/5 rounded-xl">
                      <Car size={14} className="text-[#6C63FF] mt-0.5" />
                      <div>
                        <p className="text-xs text-gray-500">Автомобиль</p>
                        <p className="text-sm text-white font-medium">{rental.car.brand} {rental.car.model}</p>
                        <p className="text-xs text-gray-500">{rental.car.plate_number}</p>
                      </div>
                    </div>
                  )}
                  <div className="flex items-start gap-2 p-3 bg-white/5 rounded-xl">
                    <Calendar size={14} className="text-yellow-400 mt-0.5" />
                    <div>
                      <p className="text-xs text-gray-500">Период</p>
                      <p className="text-sm text-white">{new Date(rental.start_date).toLocaleDateString('ru-RU')}</p>
                      <p className="text-sm text-white">— {new Date(rental.end_date).toLocaleDateString('ru-RU')}</p>
                    </div>
                  </div>
                </div>
                {rental.violations.length > 0 && (
                  <p className="text-red-400 text-xs flex items-center gap-1">
                    <AlertTriangle size={12} /> {rental.violations.length} нарушений
                  </p>
                )}
              </div>

              <div className="flex flex-col gap-2 flex-shrink-0">
                <button onClick={() => setMapModal(rental.id)}
                  className="flex items-center gap-2 px-3 py-1.5 bg-blue-500/10 border border-blue-500/20 hover:bg-blue-500/20 text-blue-400 rounded-lg text-xs transition-colors">
                  <MapPin size={12} />На карте
                </button>
                <button onClick={() => setViolationsModal(rental.id)}
                  className="flex items-center gap-2 px-3 py-1.5 bg-yellow-500/10 border border-yellow-500/20 hover:bg-yellow-500/20 text-yellow-400 rounded-lg text-xs transition-colors">
                  <AlertTriangle size={12} />Нарушения
                </button>
                <button onClick={() => setGeoEventsModal(rental.id)}
                  className="flex items-center gap-2 px-3 py-1.5 bg-purple-500/10 border border-purple-500/20 hover:bg-purple-500/20 text-purple-400 rounded-lg text-xs transition-colors">
                  <MapPin size={12} />Геозоны
                </button>
              </div>
            </div>
          </Card>
        ))}
        {!rentals?.length && !isLoading && (
          <div className="text-center py-16 text-gray-600 border border-dashed border-white/10 rounded-2xl">
            <Truck size={40} className="mx-auto mb-3 opacity-30" />
            <p>Аренд нет. Ожидайте назначения от компании.</p>
          </div>
        )}
      </div>

      {/* Map Modal */}
      <Modal isOpen={!!mapModal} onClose={() => setMapModal(null)} title="Моё положение" size="xl">
        <div className="rounded-xl overflow-hidden" style={{ height: 380 }}>
          {telemetry ? (
            <MapContainer center={[Number(telemetry.lat), Number(telemetry.lng)]} zoom={15} style={{ height: '100%', width: '100%' }}>
              <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
              <Marker position={[Number(telemetry.lat), Number(telemetry.lng)]}>
                <Popup>Скорость: {telemetry.speed} км/ч</Popup>
              </Marker>
            </MapContainer>
          ) : (
            <div className="flex items-center justify-center h-full bg-white/5 text-gray-500 text-center">
              <div><MapPin size={32} className="mx-auto mb-2 opacity-40" /><p>Нет телеметрии</p></div>
            </div>
          )}
        </div>
        {telemetry && (
          <div className="mt-3 text-sm text-gray-400">
            Скорость: <span className="text-white">{telemetry.speed} км/ч</span> · Обновлено: <span className="text-white">{new Date(telemetry.recorded_at).toLocaleTimeString('ru-RU')}</span>
          </div>
        )}
      </Modal>

      {/* Violations Modal */}
      <Modal isOpen={!!violationsModal} onClose={() => setViolationsModal(null)} title="Нарушения" size="lg">
        {violations?.length ? (
          <div className="space-y-3 max-h-80 overflow-y-auto">
            {violations.map(v => (
              <div key={v.id} className="flex items-start gap-3 p-3 bg-white/5 rounded-xl">
                <AlertTriangle size={16} className="text-red-400 mt-0.5" />
                <div>
                  <p className="text-white text-sm">{v.type === 'GEOFENCE_EXIT' ? 'Выход из геозоны' : 'Превышение скорости'}</p>
                  <p className="text-gray-500 text-xs">{new Date(v.created_at).toLocaleString('ru-RU')}</p>
                </div>
                <Badge variant="red">{v.severity}</Badge>
              </div>
            ))}
          </div>
        ) : <div className="text-center py-8 text-gray-500"><AlertTriangle size={32} className="mx-auto mb-2 opacity-30" /><p>Нарушений нет</p></div>}
      </Modal>

      {/* Geo Events Modal */}
      <Modal isOpen={!!geoEventsModal} onClose={() => setGeoEventsModal(null)} title="События геозон" size="lg">
        {geoEvents?.length ? (
          <div className="space-y-3 max-h-80 overflow-y-auto">
            {geoEvents.map(e => (
              <div key={e.id} className="flex items-start gap-3 p-3 bg-white/5 rounded-xl">
                <MapPin size={16} className={e.type === 'ENTER' ? 'text-green-400' : 'text-red-400'} />
                <div>
                  <p className="text-white text-sm">{e.type === 'ENTER' ? 'Въезд в зону' : 'Выезд из зоны'} — {e.geofence?.name}</p>
                  <p className="text-gray-500 text-xs">{new Date(e.triggered_at).toLocaleString('ru-RU')}</p>
                </div>
                <Badge variant={e.type === 'ENTER' ? 'green' : 'red'}>{e.type}</Badge>
              </div>
            ))}
          </div>
        ) : <div className="text-center py-8 text-gray-500"><MapPin size={32} className="mx-auto mb-2 opacity-30" /><p>Событий геозон нет</p></div>}
      </Modal>
    </div>
  );
};

export default DriverRentals;
